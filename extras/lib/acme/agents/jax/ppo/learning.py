...
class PPOLearner(acme.Learner):
  ...
  def __init__(
    ...
    def loss(
      ...
      observations = jnp.reshape(observations,[-1]+list(observations.shape[2:]))
      distribution_params, values = ppo_networks.network.apply(params,
                                                               observations)
      values= jnp.reshape(values,[-1,20])
      ...
      behaviour_log_probs = jnp.reshape(behaviour_log_probs,[1,-1])
      rhos = jnp.exp(log_probs - behaviour_log_probs)

      advantages = jnp.reshape(advantages,(-1,))
      rhos = jnp.reshape(rhos, (-1,20))
      rhos= jnp.mean(rhos,axis=1)
      rhos= jnp.reshape(rhos,(-1,))
    ...
    def sgd_step(
      ...
      def get_behavior_values(params: networks_lib.Params,
        ...
        o = jax.tree_map(lambda x: jnp.reshape(x, [-1] + list(x.shape[2:])),
                         observations)
        o = jnp.reshape(o,[-1]+list(o.shape[2:]))
        _, behavior_values = ppo_networks.network.apply(params, o)
        behavior_values= jnp.reshape(behavior_values,[-1,20])
        behavior_values = jnp.sum(behavior_values,axis=1)
